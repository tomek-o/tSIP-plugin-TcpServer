...
int Settings::Read(AnsiString asFileName)
{
	...
	{
		const Json::Value& jv = root["Device"];
		jv.getAString("Address", Device.asAddress);
		jv.getInt("Port", Device.port);
		if (Device.port <= 0 || Device.port >= 65535)
			Device.port = _device::DEFAULT_PORT;
		jv.getBool("HideKeepAlive", Device.bHideKeepAlive);
		jv.getAString("InitCmd", Device.initCmd);
		jv.getInt("KeepalivePeriod", Device.keepalivePeriod);
		jv.getAString("KeepaliveCmd", Device.keepaliveCmd);
	}
...
int Settings::Write(AnsiString asFileName)
{
	...
	{
		Json::Value& jv = root["Device"];
		jv["Address"] = Device.asAddress;
		jv["Port"] = Device.port;
		jv["HideKeepAlive"] = Device.bHideKeepAlive;
		jv["InitCmd"] = Device.initCmd;
		jv["KeepalivePeriod"] = Device.keepalivePeriod;
		jv["KeepaliveCmd"] = Device.keepaliveCmd;
	}
